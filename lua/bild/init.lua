local M = {}
local build_command = "./run.sh"
local Command_Window_Id = nil
local Command_Window_Buffer = nil
local is_command_window_open = false
...
function M.build()
	if is_command_window_open then
		M.close_setter_buffer()
	end
	...
	local temp_buf_win = vim.api.nvim_open_win(term_buf, true, {
		split = "right",
	})
	vim.api.nvim_win_close(temp_buf_win, false)
	vim.cmd("startinsert")
end
